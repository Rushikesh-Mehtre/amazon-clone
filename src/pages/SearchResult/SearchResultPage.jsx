...
const SearchResultPage = () => {
  ...
  const resultItems = AllProductsList.filter((item) =>
    item.keyword.includes(params.keyword)
  );
  console.log("resultItems", resultItems);
  const adArray = AdList.filter((item) =>
    item.keywords.includes(params.keyword)
  );
  ...
  return (
    <div className={styles.searchResultPage}>
      ...
      {resultItems.length > 0 ? (
        <>
          <p className={styles.NumOfResults}>
            {resultItems.length} results for <span>"{params.keyword}"</span>
          </p>
                ...
                {resultItems.map((item) => (
                  <div className={styles.searchItem}>
                    ...
                    <div className={styles.searchItemDetails}>
                      ...
                      <p className={styles.saveExtra}>
                        Save extra with No Cost EMI
                      </p>
                      <p className={styles.primeDelivery}>
                        {" "}
                        <img src={primeChecked} alt="" srcset="" />
                        FREE Delivery by Monday{" "}
                      </p>
                    </div>
                  </div>
                ))}
        ...
        <div className={styles.noResults}>
          <p>
            No results found for <strong>"{params.keyword}"</strong>{" "}
          </p>
